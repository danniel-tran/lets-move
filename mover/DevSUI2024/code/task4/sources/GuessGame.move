...
use DevSUI2024::DevSUI2024::{DEVSUI2024_FAUCET};
...
public struct Game has key {
    id: UID,
    balance: Balance<DEVSUI2024_FAUCET>
}
...
fun init(ctx: &mut TxContext) {
    let game = Game {
        id: object::new(ctx),
        balance: balance::zero<DEVSUI2024_FAUCET>()
    };
...
public entry fun deposit(game: &mut Game, coin: &mut Coin<DEVSUI2024_FAUCET>, amount: u64) {
...
public entry fun play(game: &mut Game, coin: &mut Coin<DEVSUI2024_FAUCET>, amount: u64, number: u8, r: &Random, ctx: &mut TxContext) {
    assert!(coin::value(coin) >= amount, EInsufficientFund);
    assert!(balance::value(&game.balance) >= amount, EInsufficientFund);
